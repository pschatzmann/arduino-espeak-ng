...
// holds properties of characters: vowel, consonant, etc for pronunciation rules
	...
	bool data_dictlist_mapped; // true if data_dictlist points into memory-mapped
	                            // (e.g. PROGMEM) data via espeak_mem_map() rather
	                            // than a espeak_malloc()'d buffer -- must not be
	                            // passed to espeak_free()
